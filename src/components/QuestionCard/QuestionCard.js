...
import { QuestionContainer, Question, AnswersContainer, Answer, QuestionCategory, QuestionText, Score } from './QuestionCard.styles';
import colors from '../../utils/colors';

const QuestionCard = (props) => {
  const { questioncard, onClick, score } = props;
  const { category, question, correct_answer, incorrect_answers } = questioncard;
  const [selectedAnswer, setSelectedAnswer] = useState(null);
  const shuffledAnsweres = useMemo(() => [correct_answer, ...incorrect_answers].sort(() => 0.5 - Math.random()), [correct_answer, incorrect_answers]);
